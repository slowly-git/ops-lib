...
if [ $# -lt 3 ]
then
  echo "parameter recev:$# $* para-1:$1 para-2:$2 para-3:$3 para-4:$4 para-5:$5 para-6:$6 para-7:$7 para-8:$8 Usage: $COMMAND_NAME host port zabbix_name"
  exit 1
fi
HOST=$1
PORT=$2
...
check ()
{
	...
	if [[ $VALUE == "set" ]]
	then
		status=`redis-cli -h $HOST -p $PORT -n 1 set checktest 11111`
		if [[ $status == 'OK' ]]
		then
			echo "OK"
		else
	...
	then
		status=`redis-cli -h $HOST -p $PORT -n 1 get checktest`
		if [[ $status == '11111' ]]
		then
			echo "OK"
		else
	...
	then
		status=`redis-cli -h $HOST -p $PORT -n 1 expire checktest 10`
		if [[ $status == 1 ]]
		then
			echo "OK"
		else
...
echo "$ZABBIX_NAME redis.set.status `check 'set'`" >>$DATA_FILE
echo "$ZABBIX_NAME redis.get.status `check 'get'`" >>$DATA_FILE
echo "$ZABBIX_NAME redis.expire.status `check 'expire'`" >>$DATA_FILE
