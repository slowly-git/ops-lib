...
func main() {
        ...
        ss := strings.Contains(strings.TrimSpace(hn), "c360in.com")
        ...
        if len(region) > 2 && ss != true && (strings.TrimSpace(hn) == "localhost.localdomain" || ss1 == true) {
                ...
                real_get_tag_name := strings.Contains(strings.TrimSpace(final), "c360in.com")
...
func Set_Local_HostName(hostname string) (err error) {
        ...
        err = sh.Command("hostname", nis).Run()
...
func Final_HostName_Ensure(region string) (hostname string) {
        ...
        if strings.TrimSpace(tgs) == "" {
                ...
                if strings.Contains(tgs, "php") || strings.Contains(tgs, "app") {
                        hostname = strings.TrimSpace(tgs) + "-" + rdm + ".c360in.com"
                } else {
                        hostname = strings.TrimSpace(tgs) + ".c360in.com"
                }
        }
        return hostname
}
...
func GetTagFromId(region string, insid string) (tag string) {
        svc := ec2.New(session.New(), AwsConfig(region))
